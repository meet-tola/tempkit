...
function TemplateCard({
  ...
  return (
    <div onClick={onUse} className="cursor-pointer hover:shadow-sm transition-shadow">
...
export default function DashboardPage() {
  ...
  const loadData = async () => {
    try {
      const [templatesData, projectsData, documentsData, usageData] =
        await Promise.all([
          getTemplates(),
          getProjects(),
          getDocuments(),
...
const handleUseTemplate = async (template: any) => {
  try {
    ...
    formData.append("title", `Copy of ${template.name}`);
    formData.append("templateId", template.id);
    formData.append("content", template.content);
    
    // If you want to associate with a project, add this:
    ...
    if (result.success) {
      ...
      router.push(`/editor?document=${result.document.id}`);
    } else {
      alert(result.error || "Failed to create document from template");
    }
  } catch (error) {
          ...
          <div className="w-full max-w-2xl">
            <div className="text-center mb-8">
              ...
              <p className="text-slate-600">
                Describe your document needs and I'll generate it for you.
              </p>
              {userUsage?.plan === "FREE" && (
                <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <p className="text-sm text-amber-800">
                    <strong>{5 - (userUsage?.aiUsage || 0)}</strong> AI
                    generations remaining on free plan
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-3">
                <Select
                  ...
                  onValueChange={setAiDocumentType}
                  disabled={!canUseAi}
                >
